...
def main(filePath):
    ...
    CheckerBoard = Board(image,8,8)
    orig = image.copy()
    image = CheckerBoard.get_image()
    ...
    board = [[0 for i in range(row)] for r in range(column)]
    for r,row in enumerate(range(len(points_sorted)-1)):
        for c,column in enumerate(range(len(points_sorted[row])-1)):
            ...
            circles = DetectCircles(box, dp=1, mindist=w, cannyThreshold=100, accumulator=10, minRadius=w/3, maxRadius=w/2)
